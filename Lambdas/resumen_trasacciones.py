...
def generar_y_mostrar_resumen():
    ...
    for mes, num_transacciones in resumen['Número de transacciones por mes'].items():
        resumen_t += f"Número de transacciones en {mes}: {num_transacciones}\n"

    for mes, monto_promedio in resumen['Monto promedio del crédito por mes'].items():
        resumen_t += f"Monto promedio del crédito en {mes}: {monto_promedio:.2f}\n"

    for mes, monto_promedio in resumen['Monto promedio del débito por mes'].items():
        resumen_t += f"Monto promedio del débito en {mes}: {monto_promedio:.2f}\n"

    resumen_t += f"Importe medio del débito: {resumen['Importe medio del débito']:.2f}"
